...
const currentWidgetUri = "ui://aevren/heartbar-v2.html";
const widgetUris = ["ui://aevren/heartbar-v1.html", currentWidgetUri];

const clamp = (value, min, max) => Math.max(min, Math.min(max, value));

function makeServer() {
  const server = new McpServer(
    { name: "aevren-heartbar", version: "2.0.0" },
    {
      instructions:
...
app.get("/health", (_req, res) => {
  res.json({ ok: true, name: "aevren-heartbar", version: "2.0.0" });
});
